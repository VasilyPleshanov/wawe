...
class Player {
    ...
    initTimelineListeners() {
        ...
        // new
        // const currentLine = this.player.querySelector('.j-line-current')
        // currentLine.addEventListener('click', (event) => {
        //     const { left } = event.target.getBoundingClientRect()
        //     this.video.currentTime = this.calcNeededLine1(event, left)
        //     console.log(this.calcNeededLine(event, left))
        // })
    ...
    formatTime(seconds) {
        return `${Math.floor(seconds / 60)}:${this.fixNumber(Math.floor(seconds))}`
    }
    ...
    checkFullscreen() {
        ...
        const icon = this.player.querySelector('.j-fullscreen-icon')
    ...
    calcNeededLine(event, left) {
        ...
        // console.log(event.clientX, left, event.target.offsetWidth)
        return this.video.duration * needPercent
    }

    // new
    // calcNeededLine1(event, left) {
    //     const needPercent = ((event.clientX - left) / event.target.offsetWidth)
    //     console.log(event.clientX, left, event.target.offsetWidth)
    //     return this.video.duration * needPercent
    // }
    ...
    updateVolumeInput() {
        ...
        toggleClasses.remove(VOLUME_ICONS.high, VOLUME_ICONS.low, VOLUME_ICONS.off, VOLUME_ICONS.xmark)

        if (this.video.volume > 0.66) {
            toggleClasses.add(VOLUME_ICONS.high)
        } else if (this.video.volume > 0.33) {
            toggleClasses.add(VOLUME_ICONS.low)
        } else if (this.video.volume > 0) {
            toggleClasses.add(VOLUME_ICONS.off)
        } else if (this.video.volume === 0) {
            toggleClasses.add(VOLUME_ICONS.xmark)
        }
    }
